...
{
    if(mum->joueurActif==mum->joueurHumain)
    {
        if(!mum->partieFinie)
        {
            ...
            if(this->estVide())
            {
                ...
                if(mum->joueurActif->getNbJetons()>0)
                {
                    ...
                    mum->envoyerAuServeur(this->ligne,this->col);
    ...
    if(mum->joueurActif==mum->joueurHumain)
    {
        ...
        if(emplacementRelache!=NULL && emplacementRelache->inherits("QEmplacement") && emplacementRelache->estVide() && jouxte(emplacementRelache))
        {
            if(mum->emplacementDeplace!=NULL)
            {
                ...
                //qDebug()<<QString::number(posDeplx)+" "+QString::number(posDeply)+" "+QString::number(posRelx)+" "+QString::number(posRely);
                mum->envoyerAuServeur(posDeplx,posDeply,posRelx,posRely);
